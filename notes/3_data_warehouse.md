...
You may choose clustering over partitioning when partitioning results in a small amount of data per partition (for clustering we must to partition table before),
when partitioning would result in over 4000 partitions or if your mutation operations modify the majority of partitions in the table frequently (for example, writing to the table every few minutes and writing to most of the partitions each time rather than just a handful).
...
we can set partition expiration days / required partition filter
```sql
ALTER TABLE
...
use data_trunc iun case we have more then 4000 day partition
```sql
...
[Create your customized partition](https://medium.com/google-cloud/partition-on-any-field-with-bigquery-840f8aa1aaab)
<br> BigQuery has a lot of built-in functions and one of them is Farm-Fingerprint hash function
<br>
  ![ customized partition](images/intteger_partition.png)
...
* ***Jupiter***: the network that connects Dremel and Colossus.
  * Jupiter is an in-house network technology created by Google which is used for interconnecting its datacenters > ITB per second.
